<template>
  <div class="grid-table">
    ...
    >
      ...
      <template v-for="(column, columnIndex) in columnsVisible">
    ...
    <div class="grid-table__pagination">
      <slot name="pagination" :loading="loading">
        <el-pagination
          v-if="showPagination"
  ...
  methods: {
    ...
    dataFilterHandler(formParams) {
      ...
      if (validParamKeys.length > 0) {
        ...
        this.tableData = this.showPagination ? this.dataFilter(validData) : validData;
        this.total = validData.length;
        this.loading = false;
      } else {
        const { sortOrder } = this.sortParams;
        this.total = cacheLocalData.length;
        this.tableData = this.showPagination
          ? this.dataFilter(sortOrder ? sortLocalData : cacheLocalData)
          : cacheLocalData;
        this.loading = false;
      }
    },
    ...
    loadLocalData(data) {
      ...
      if (autoload) {
        this.tableData = this.showPagination ? this.dataFilter(cacheData) : cacheData;
